...
- [x] **Independent Feature Semantic Versioning Contract (`version.ts`, `AGENTS.md`, `DECISIONS.md ADR-040`)**: `QUEUE_WORKFLOW_VERSION = '0.2.1'` with mandatory SemVer bump on every push (patch for UI/UX/bug fixes, minor for features, major for schema breaking/BETA graduation).

### Upcoming Queue Workflow Candidates (Planned — Not Yet Implemented)
- [ ] **Incompatible Wire Connection Validation & Link Prevention**: Prevent connecting cables between incompatible tool ports (e.g. text output into file-only input, or tools lacking required capabilities) with instant visual warning toast rejecting the wire drop, while keeping canvas tool placement completely unconstrained.
