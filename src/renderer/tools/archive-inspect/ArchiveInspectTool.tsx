...
export default function ArchiveInspectTool() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-6xl space-y-6 px-8 py-8">
      {/* Header */}
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-[12.5px] text-dim">
            Inspect, search, and preview files inside archives in-memory without extracting them to
            disk.
          </p>
        </div>
        ...
        {archivePath && inspectResult && (
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 rounded-md border border-line bg-surface/80 px-3 py-1.5 text-[12px] text-dim">
      ...
      {!archivePath ? (
        <DropZone
          ...
          hint="Drop a .zip archive to inspect its contents without extracting"
          multiple={false}
          className="h-64"
        />
      ) : loading ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex shrink-0 items-center gap-2 text-[11px] text-faint">
                          {!entry.isDirectory && <span>{formatBytes(entry.uncompressedSize)}</span>}
