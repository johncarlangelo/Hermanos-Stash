...
- **Headless Stress Testing & Diagnostic Evaluation (122 Queries)**: Implemented comprehensive test harness covering 122 queries across 7 functional categories. MiniLM vector engine executes in 2.6 ms average latency (P95: 4.1 ms) with 100.0% clean pass rate (122/122) across single tool, format-specific, broad ambiguity, composite pipeline, conversational, and gibberish queries.
- **Domain-Aware Ambiguity Gating & Format Affinity**: Resolved false ambiguities by introducing whole-word regex boundary matching on ambiguity cluster keywords, domain pre-checks (bypassing broad questions if explicit formats or extensions are provided), and format affinity boosting (+0.10) for matching extensions.
- **Automated Catalog Parity & Regression Verifier (`npm run router:check`)**: Shipped `scripts/check-routing-parity.mjs` and companion vitest assertions in `semantic-router.test.ts` guaranteeing 100% parity across `src/renderer/tools/index.ts`, `TOOL_ROUTING.md`, and `tool-embeddings.json`. Prevents adding future tools (Tool #79+) without decision routing triggers and normalized 384-d vectors.
- **Verification & Store**: Wired IPC `chatbot:semantic-route` and `chatbot:unload-model` into `useChatbot` with automatic heuristic fallback; verified with dedicated test suites in `semantic-router.test.ts` (12/12), `dependencies.test.ts` (4/4), `conversational-guards.test.ts` (27/27), and `chatbot.test.ts` (8/8); all 984 project tests across 97 files pass (100%); zero typecheck, lint, or format issues.
