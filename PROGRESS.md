...
- **Verification & Store**: Wired IPC `chatbot:semantic-route` and `chatbot:unload-model` into `useChatbot` with automatic heuristic fallback; verified with dedicated test suites in `semantic-router.test.ts` (5/5), `dependencies.test.ts` (4/4), and `chatbot.test.ts` (5/5); all 946 project tests across 95 files pass (100%); zero typecheck, lint, or format issues.
